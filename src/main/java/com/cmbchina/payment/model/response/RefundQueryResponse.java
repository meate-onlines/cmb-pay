...
public class RefundQueryResponse extends BaseResponse {
    ...
    /**
     * 商户退款订单号（returnCode为SUCCESS时返回，可选）
     * 请求报文中的商户退款订单号，有则返回
     */
     ...
     * 银联通道且有商品优惠时返回，字段说明详见2.6.3.银联优惠信息
     */
    ...
    /**
     * 退单原始金额（returnCode和respCode为SUCCESS时返回，可选）
    ...
    /**
     * 退单优惠金额（returnCode和respCode为SUCCESS时返回，可选）
     ...
     * 第三方系统返回的交易完成日期，此日期仅在退款成功时返回，不保证是实际清算日期，格式为yyyyMMdd
     */
     ...
     * 支付宝和微信通道的成功退款交易返回此字段
     * 银联通道的退款交易无论成功与否，均不返回此字段
     */
     ...
     * json字符串，字段说明详见2.7.2.4合约退款响应内容contractRefundResp
     */
